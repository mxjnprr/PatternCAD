...
- `/src/core/Document.h/cpp` - Layer management fully integrated:
  - QStringList m_layers, QMap m_layerVisibility, QMap m_layerColors
  - Methods: addLayer(), removeLayer(), renameLayer(), setActiveLayer()
  - Layer visibility: isLayerVisible(), setLayerVisible()
  - Layer colors: layerColor(), setLayerColor()
  - Signals: layerAdded, layerRemoved, layerRenamed, activeLayerChanged, layerVisibilityChanged
- Layer system integrated with Document lifecycle (save/load would persist layers)
- Objects assigned to layers (checked via geometry code)
- LayersPanel uses all layer methods successfully
